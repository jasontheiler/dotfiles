...
WORK_DIR=$(mktemp -d /tmp/tmux-patched.XXXXXX)
echo ">> building tmux $TMUX_VERSION in $WORK_DIR"

git clone --depth=1 --branch="$TMUX_VERSION" https://github.com/tmux/tmux.git "$WORK_DIR"
cd "$WORK_DIR"
